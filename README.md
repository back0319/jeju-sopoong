...
제주 소풍 매장에서 **재료를 고르고 직접 김밥을 만드는 체험**을 위한 모바일 주문 웹입니다. 고객은 공통 QR로 접속해 설문과 재료 선택을 마치고 주문번호를 받습니다. 직원은 카운터에서 주문을 확인하고 오프라인 결제와 재료 수령을 처리합니다.
...
```mermaid
flowchart LR
  QR[공통 QR · 언어 선택] --> Intro[소개 · 내부 테스트 동의]
  Intro --> Survey[설문 8문항]
  Survey --> Origin[제주 원물 확인]
  Origin --> Build[기본 재료 · 토핑 · 패키지]
...
고객 화면은 `/`, 관리자 화면은 `/admin`입니다. 관리자에는 운영, 재고·메뉴, 콘텐츠, 데이터 탭이 있습니다.
...
관리자 생성에는 `.env.admin.local`의 `ADMIN_EMAIL`, `ADMIN_PASSWORD`를 사용합니다. 관리자 비밀번호는 최소 16자로 설정하고 다음을 실행합니다.

```bash
npm run admin:create
```

계정 값은 코드나 시드에 포함되지 않습니다. `.env*local`은 Git에서 제외됩니다. 공개 회원가입을 비활성화하고 공용 관리자 계정 하나로 운영합니다.
...
5. **영업 후:** 데이터 탭에서 기간·상태·언어·재료로 조회하고 CSV를 내려받습니다. 주문당 한 행에 모든 구성과 설문 응답을 함께 기록합니다.

기본 김밥 5,000원, 토핑별 3,000원입니다. 기본 재료를 빼도 가격은 같습니다. 패키지는 10,000원 추가이며 토핑 값도 더합니다. 패키지·라면·음료·굿즈는 초기에 비활성이고 미정 가격을 임의로 넣지 않습니다.

콘텐츠 탭에서 소개, 원물 설명·사진 URL, 언어별 유튜브 링크를 입력합니다. 원물 일러스트의 순서는 PORK → TURBAN SHELL → FERNBRAKE → CARROT입니다.
...
- **복원:** 주문 완료 시 날짜·번호·저장 시각만 localStorage에 저장합니다. 내역은 서버에서 다시 읽고 4시간 또는 KST 날짜 변경 시 만료합니다. 공개 조회는 설문·동의 데이터를 반환하지 않습니다.
...
**브랜드 로고와 일러스트는 제주 소풍의 자산이며 코드 라이선스에 포함되지 않습니다.** 제공된 원본은 `assets/brand/`, 웹용 복사본은 `public/brand/`에 있습니다. 코드 라이선스는 아직 지정하지 않았습니다.

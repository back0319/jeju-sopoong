...
| 실시간 | Supabase Realtime: 주문 상태·재고·메뉴·연구 데이터 갱신, 연결 실패 시 자동 조회 |
...
- 고객 주문 상태는 `order:<주문 UUID>` 채널의 변경 신호를 받은 뒤 공개 주문 조회 API로 다시 확인합니다. 신호에는 설문 응답·동의·주문 구성·취소 사유를 넣지 않습니다. 주문 테이블의 익명 직접 조회 차단은 유지합니다.
- 고객 재고·가격·메뉴·소개 콘텐츠, 관리자 주문·재고·메뉴와 연구 데이터는 DB 변경 이벤트로 갱신합니다. 진행 중 설문 버전과 응답, 장바구니 재료 선택은 자동 변경하지 않습니다.
- 연결 정상 시 60초 보조 조회로 누락을 보완하고, 연결 실패 시 고객·연구 데이터는 5초, 관리자 운영은 2초마다 조회합니다. 화면 복귀·네트워크 복구 시에도 다시 조회합니다.
- 완료 처리 되돌리기의 5초 제한과 주문 저장의 4시간 만료는 업무 규칙이므로 유지합니다.
